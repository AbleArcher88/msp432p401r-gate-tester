...
static GATE2_IN1: Mutex<RefCell<Option<hal::gpio::portc::P5_4<Output>>>> = Mutex::new(RefCell::new(None));
...
fn main() -> ! {
    ...
    loop {
        let dev_table: [bool; 4];
        if DEV_IND.load(Ordering::Relaxed) == 0 {
            bled.set_low();
            rled.set_high();
            dev_table = [false, true, true, true];
        } else {
            rled.set_low();
            bled.set_high();
            dev_table = [false, false, false, true];
        }
